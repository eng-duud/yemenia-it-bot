...
class Database:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    username TEXT,
                    role TEXT DEFAULT 'user', -- 'user', 'delegate', 'admin'
                    level_id INTEGER, -- For delegates to restrict access to a specific level
                    is_banned INTEGER DEFAULT 0, -- 0 for active, 1 for banned
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS content (
                    ...
                    url TEXT, -- For links
                    added_by INTEGER, -- user_id of the person who added it
    ...
    def get_user(self, user_id):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT user_id, username, role, level_id, is_banned FROM users WHERE user_id = ?", (user_id,))
            return cursor.fetchone()

    def update_user_role(self, user_id, role, level_id=None):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET role = ?, level_id = ? WHERE user_id = ?", (role, level_id, user_id))
            conn.commit()

    def set_ban_status(self, user_id, is_banned):
        with self.get_connection() as conn:
            cursor = conn.cursor()
    ...
    def get_all_users(self):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT user_id, username, role, is_banned FROM users")
    ...
    def add_content(self, subject_id, content_type, title, added_by, file_id=None, url=None):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO content (subject_id, type, title, added_by, file_id, url) VALUES (?, ?, ?, ?, ?, ?)",
                (subject_id, content_type, title, added_by, file_id, url)
            )
            conn.commit()
